...
class Module_ontology_v1x_Tests(t_utils.TestUtils):
    async def test_addDynamicDomains_create(self):
        """
        Validate the appropriate nodes are created when a network.dynamic.domain tag is
        ...
        async with self.getTestCoreWithPkg() as core:
            q = """ 
                ...
                for $n in $mod.addDynamicDomains($ont_result_dict) {
                    yield $n
                }
                """
            opts = {
            ...
            expected = [
                {
                    "form": "inet:dns:request",
                    "valu": s_common.guid(
                        ("assemblyline4", sha256, "ncsi.4-c-0003.c-msedge.net")
                    ),
                    "query:name": "ncsi.4-c-0003.c-msedge.net",
                    "sandbox:file": f"sha256:{sha256}",
                },
                {
                    "form": "inet:dns:request",
                    "valu": s_common.guid(
                        ("assemblyline4", sha256, "time.windows.com")
                    ),
                    "query:name": "time.windows.com",
                    "sandbox:file": f"sha256:{sha256}",
                },
            ]
                ...
                self.isin(
                    {
                        "form": nod.ndef[0],
                        "valu": nod.ndef[1],
                        "query:name": nod.props.get("query:name"),
            ...
            for item in expected:
                fqdn_nodes = await core.nodes(f'inet:fqdn={item.get("query:name")}')
    ...
    async def test_addDynamicDomains_NoCreateSinceNoTag(self):
        """
        Validate an empty list is returned when there are no network.dynamic.domain tags
    ...
    async def test_addDynamicURIs_invalidValue(self):
        """
        Validate no nodes are created when only invalid values are present in the network.dynamic.domain tag.
        """
        async with self.getTestCoreWithPkg() as core:
            q = """ 
                $mod = $lib.import(al4.ontology.v1x)
