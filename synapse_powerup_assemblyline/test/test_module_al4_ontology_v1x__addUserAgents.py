...
class Module_ontology_v1x_Tests(t_utils.TestUtils):
    async def test_addUserAgents_create(self):
        """
        Validate an inet:http:request:header node is created.
            - light edges as expected
        """
        async with self.getTestCoreWithPkg() as core:
            ...
            q = """ 
                ...
                for $n in $mod.addUserAgents($ont_result_dict) {
                    yield $n
                }
                """
            opts = {
    ...
    async def test_addUserAgents_NoCreateSinceNoTag(self):
        """
        Validate an empty list is returned when there are no network.user_agent tags
