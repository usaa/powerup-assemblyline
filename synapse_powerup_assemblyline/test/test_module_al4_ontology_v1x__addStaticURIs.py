...
class Module_ontology_v1x_Tests(t_utils.TestUtils):
    async def test_addStaticURIs_create(self):
        """
        Validate an inet:url node is created.
        """
        async with self.getTestCoreWithPkg() as core:
            ...
            q = """ 
                ...
                for $n in $mod.addStaticURIs($ont_result_dict) {
                    yield $n
                }
                """
            opts = {
            ...
            expected = [
                {"form": "inet:url", "valu": "http://foo.local?id=123"},
                {"form": "inet:url", "valu": "https://bar.local/"},
            ]
    ...
    async def test_addStaticURIs_NoCreateSinceNoTag(self):
        """
        Validate an empty list is returned when there are no network.static.uri tags
    ...
    async def test_addStaticURIs_invalidValue(self):
        """
