...
class Module_ontology_v1x_Tests(t_utils.TestUtils):
    async def test_addFilepathFromTags_create(self):
        """
        Validate file:filepath nodes are created
        """
        async with self.getTestCoreWithPkg() as core:
            ...
            q = """ 
                ...
                for $n in $mod.addFilepathFromTags($ont_result_dict) {
                    yield $n
                }
                """
            opts = {
            ...
            expected = [
                {"form": "file:filepath", "valu": (f"sha256:{sha256}", "pskill64.exe")},
                {"form": "file:filepath", "valu": (f"sha256:{sha256}", "pskill.exe")},
            ]
    ...
    async def test_addFilepathFromTags_NoCreateSinceNoTag(self):
        """
        Validate an empty list is returned when there are file.names found in the
        ontology result.
        """
        async with self.getTestCoreWithPkg() as core:
            q = """ 
