...
class Module_setup_admin_Tests(t_utils.TestUtils):
    async def test_getGlobalTagPrefix(self):
        """
        Validate Tag Prefix is returned
        """
    ...
    async def test_getGlobalTagPrefix_returns_null(self):
        """
        Verify returns $lib.null when tag prefix is not set
        """
        async with self.getTestCoreWithPkg() as core:
    ...
    async def test_getGlobalTagPrefix_raises_AuthDeny_noAsRootPerms(self):
        """
    ...
    async def test_getGlobalTagPrefix_powerupUserInAdminGroup(self):
        """
